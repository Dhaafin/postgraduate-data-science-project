...
| 2026-05-18 |   Dev   | fix: synchronize missing database sequence pointer to prevent duplicate id conflicts | Ran `setval` to align the Supabase auto-increment sequence with `MAX(id)` after migration disrupted the pointer. |
| 2026-05-18 |   Dev   | fix: add identity generation for id column and extract origin_province | Restored the missing `GENERATED BY DEFAULT AS IDENTITY` sequence on Supabase `id` column and enhanced the MusicBrainz scraper to extract both `origin_city` and `origin_province`. |
